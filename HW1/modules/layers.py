...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            self.var = np.power(self.input_mean, 2).sum(axis=0) / B
    ...
    def update_grad_parameters(self, input: np.ndarray, grad_output: np.ndarray):
        ...
        # super().update_grad_parameters(input, grad_output)
        if self.affine:
            self.grad_weight += (grad_output * self.norm_input).sum(axis=0) # (num_features,) 
            self.grad_bias += grad_output.sum(axis=0) # (num_features,)
